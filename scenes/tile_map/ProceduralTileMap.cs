...
public partial class ProceduralTileMap : TileMap
{
    ...
    private bool IsSpawnLocationValid(int x, int y, int startX, int startY, Vector2I areaSize)
    {
        if (
            x < 0
            || x > worldData.worldDimmensions.X - areaSize.X
            || y < 0
            || y > worldData.worldDimmensions.Y
            || visited[x, y]
        )
        {
            return false;
        ...
        if (
            startX + 1 < areaSize.X
            && x + 1 < worldData.worldDimmensions.X - areaSize.X
            && !visited[x + 1, y]
        )
        {
        ...
        if (
            startY + 1 < areaSize.Y
            && y + 1 < worldData.worldDimmensions.Y - areaSize.Y
            && !visited[x, y + 1]
        )
        {
    ...
    public void Generate()
    {
        foreach (int x in GD.Range(worldData.worldDimmensions.X))
        {
            foreach (int y in GD.Range(worldData.worldDimmensions.Y))
            {
                ...
                int randomGroundTile = Math.Clamp(
                    (int)Math.Floor(Math.Abs(groundNoiseValue * worldData.groundValue)),
                    0,
                    19
                );
